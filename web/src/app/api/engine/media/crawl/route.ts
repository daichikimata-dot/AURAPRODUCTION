...
export async function POST() {
    try {
        const res = await fetch(`${engineUrl}/media/crawl`, {
            method: 'POST',
            headers: {
        ...
        if (!res.ok) {
            ...
            console.error(`Engine API Error (Media Crawl): ${res.status} ${errorText}`);
        ...
        console.error("API Proxy Error [Media Crawl]:", error);
